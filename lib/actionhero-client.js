...
actionheroClient.prototype.reconnect = function(){
  ...
  if(self.reconnectAttempts > self.params.reconnectAttempts){
    self.emit('error', new Error('maximum reconnectAttempts reached'));
  }else if(self.params.reconnectTimeout && self.params.reconnectTimeout > 0){
    setTimeout(function(){
      self.connect();
...
actionheroClient.prototype.send = function(str){
  if (!this.connection.destroyed) this.connection.write(str + "\r\n");
};
